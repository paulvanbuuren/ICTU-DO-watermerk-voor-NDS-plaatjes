...
//if (empty($_POST['WATERMERK_body']))
//  die('Geen tekst ingevoerd<br /><a href="./index.php">opnieuw</a>');

//if (empty($_POST['WATERMERK_titel']))
//  die('Geen tekst ingevoerd<br /><a href="./index.php">opnieuw</a>');
...
if ( isset( $_FILES["file"] ) ) {
    ...
    if ( $_FILES["file"]["error"] > 0 ) {
        ...
        writedebug( "File size: " . ( $_FILES["file"]["size"] / 1024 ) . " kB<br>" );
...
$path_to_watermerk = $imagepath . "logo.png";
...
$resizefactor           = 1;
$body_size              = 36;
$watermerk              = isset( $_POST['watermerken'] ) ? $_POST['watermerken'][0] : 'meh';
$filename               = seoUrl( "digitale overheid - " . '-' . date( "Y" ) . " " . date( "m" ) . "-" . date( "d" ) . "-" . date( "H" ) . "-" . date( "s" ) . "-" . date( "i" ) ) . ".png";
$destimagepath          = $outpath . $filename;
$destimageurl           = $_SERVER['REQUEST_SCHEME'] . $_SERVER['HTTP_HOST'] . "/posters/" . $filename;

echo '<h1>$watermerk: ' . $watermerk . '</h1>';
...
if ( ! file_exists( $outpath . $filename ) ) {
    ...
    list( $watermerk_width, $watermerk_height ) = getimagesize( $path_to_watermerk );
    $watermerk  = imagecreatefrompng( $path_to_watermerk );
    $logo_box_x = 0;
    // ================================================================================================================================
    $offset_y = 200;
    $offset_x = $offset_y;
    ...
    $total_image_height         = ( ( ( $bodybox_height > $upload_image_resize_height ) ? $bodybox_height : $upload_image_resize_height ) ) + $watermerk_height + ( 2 * $span );
    ...
    if ( $upload_image_resize_height < ( $bodybox_height - $titlebox_height ) ) {
        ...
        $upload_image_resize_width  = ( $upload_image_resize_width * $resizefactor );
        $upload_image_resize_height = ( $upload_image_resize_height * $resizefactor );
        $total_image_height         = ( ( ( $bodybox_height > $upload_image_resize_height ) ? $bodybox_height : $upload_image_resize_height ) ) + $titlebox_height + $watermerk_height;
    ...
    $logo_x             = ( $total_image_width - ( $resizefactor * ( $span / 2 ) ) ) - $watermerk_width;
    $logo_y             = ( $total_image_height - ( $resizefactor * ( $span / 2 ) ) ) - $watermerk_height;

    // ================================================================================================================================

    $upload_image_resize_width = 1600;
    $total_image_width = 1600;
    $total_image_height = 900;


    $tmp = @imagecreatetruecolor( $upload_image_resize_width, $total_image_height ) or die( 'Cannot Initialize new GD image stream for temp' );
    $outputimage = @imagecreatetruecolor( $total_image_width, $total_image_height ) or die( 'Cannot Initialize new GD image stream for output' );

    echo '<h1>$total_image_width: ' . $total_image_width . ', $total_image_height: ' . $total_image_height . '</h1>';
    ...
    imagecopy( $outputimage, $watermerk, $logo_x, $logo_y, 0, 0, $watermerk_width, $watermerk_height );
    ...
    <div class="starter-template">
        <div class="panel panel-default">
            <div class="panel-body">
                <div class="row">
                    <div class="col-md-12">
                        <p><a href="index.php"><?php echo WATERMERK_BACK ?></a></p>
                        <h1><?php echo WATERMERK_TITLE ?></h1>
                        <p class="lead"> <?php echo WATERMERK_READY ?> filename: <?php echo $filename ?> </p>
                        <a href="<?php echo htmlspecialchars( $destimageurl ) ?>" download="<?php echo $filename ?>">
                            <img src="<?php echo $destimageurl ?>" alt="<?php echo $filename ?>" class="result"> </a>
                        <p><a href="index.php"><?php echo WATERMERK_BACK ?></a></p>
                    </div>
                </div>
            </div>
